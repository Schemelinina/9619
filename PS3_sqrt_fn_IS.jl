...
using Plots
plot(x, y1, label = "sqrt(c)_monomial")
...
using Plots
plot!(x1, y2, label = "sqrt(c)",title="√c of different sizes_monomial")
...
using Plots
plot(p,x_ext, title="sqrt fn extrapolation")

f_ext(x_ext)=sqrt.(x_ext)
plot!(f_ext,x_ext)
...
plot(xi,interp, title="sqrt_Newton")

#Extrapolating
plot!(f_ext,x_ext, title="sqrt function_Newton_extrapolation")
...
plot(x,int,label="cubic spline_sqrt function")
...
plot!(x_ext,naturalspline_ext,label="cubic spline_cubic spline_extrapolation")