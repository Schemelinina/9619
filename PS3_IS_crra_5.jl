...
#CRRA, sigma=5 function
    ...
    using Plots
    plot(x, y1, label = "crra,5_monomial")
    ...
    using Plots
    plot!(x1, y2, label = "crra,5, monomial of diff sizes")
    ...
    using Plots
    plot(p,x_ext, title="crra,5, monomial, extrapolation")
    
    f_ext(x_ext)=x_ext.^(1-5)/(1-5)
    plot!(f_ext,x_ext)
    ...
    plot(xi,interp, title="crra,5_Newton")
    #Extrapolating
    plot!(f_ext,x_ext)
    ...
    plot(x,int,label="cubic spline, crra5")
    ...
    plot!(x_ext,naturalspline_ext,label="cubic spline_crra,5_extrapolation")